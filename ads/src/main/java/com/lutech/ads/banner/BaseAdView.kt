...
internal abstract class BaseAdView(
    ...
    private lateinit var onFailedCallback: () -> Unit

    fun loadAd(onFailedCallback: () -> Unit) {
        log("LoadAd ...")
    ...
    protected abstract fun loadAdInternal(onFailedCallback: () -> Unit,onDone: () -> Unit)
    ...
    private fun scheduleNextBannerRefreshIfNeed() {
        ...
        refreshHandler.postDelayed({ loadAd(onFailedCallback) }, delay)
    }
